...
/**
 * Siembra los cantones y distritos reales de Costa Rica para los selects encadenados
 * del perfil de arrendador/inquilino (cr_cantons, cr_districts).
 *
 * Los códigos de 2 dígitos NO se inventan: se derivan de la posición de cada cantón/
 * distrito dentro de su provincia/cantón, siguiendo el mismo orden canónico oficial del
 * INEC (División Territorial Administrativa) que usa Hacienda en el XSD de factura
 * electrónica — orden verificado contra el catálogo real (ej. San José: 101 San José,
 * 102 Escazú, 103 Desamparados... coincide exactamente con este arreglo). Los nombres de
 * provincia/cantón/distrito son los mismos que ya se usan en
 * App\Http\Controllers\PropertyController::locationData().
 */
class CrLocationCatalogSeeder extends Seeder
{
    public function run(): void
    {
        $provinceCodes = DB::table('cr_provinces')->pluck('code', 'name');

        foreach ($this->catalog() as $provinceName => $cantons) {
            $provinceCode = $provinceCodes[$provinceName] ?? null;
            ...
            foreach ($cantons as $cantonName => $districts) {
                $cantonPosition++;
                $cantonCode = str_pad((string) $cantonPosition, 2, '0', STR_PAD_LEFT);
                ...
                foreach ($districts as $districtName) {
                    $districtPosition++;
                    $districtCode = str_pad((string) $districtPosition, 2, '0', STR_PAD_LEFT);
        ...
        return [
            ...
            'Cartago' => [
                'Cartago' => [
                    ...
                    'Aguacaliente o San Francisco', 'Guadalupe o Arenilla', 'Corralillo',
                    'Tierra Blanca', 'Dulce Nombre', 'Llano Grande', 'Quebradilla',
                ],
                'Paraíso' => [
