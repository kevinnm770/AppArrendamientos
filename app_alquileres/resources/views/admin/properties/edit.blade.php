...
@section('content')
    ...
    <section class="section">
        <form class="card" id="property-form" method="POST" action="{{ route('admin.properties.edit.update', $property->id) }}" enctype="multipart/form-data">
            @csrf
            @method('PATCH')
            ...
            <div class="card-body">
                <div class="row g-3 mb-4">
                    <div class="col-8 col-lg-4">
                        ...
                        @if ($hasLockedAgreement)
                            <small class="d-block mt-1" style="font-size:10pt;color:#4CD2D9;">El estado queda bloqueado en <strong>Ocupada</strong> por un contrato en estado accepted.</small>
                        @endif
                        @error('status')
                    ...
                    <div class="col-12 col-lg-6">
                        ...
                        <select class="form-select @error('service_type') is-invalid @enderror" id="service_type" name="service_type" required>
                            ...
                            <option value="home" @selected(old('service_type', $property->service_type) === 'home')>Vivienda</option>
                            <option value="commercial" @selected(old('service_type', $property->service_type) === 'commercial')>Local comercial</option>
                        </select>
                        @error('service_type')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                    ...
                    <div class="row mt-4">
                        <div class="col-0 col-lg-4">
                            ...
                            <select class="form-select mt-2 @error('currency') is-invalid @enderror" id="currency" name="currency" style="width: max-content;" required>
                                <option value="CRC" @selected(old('currency', $property->currency) === 'CRC')>Colones (₡)</option>
                                <option value="USD" @selected(old('currency', $property->currency) === 'USD')>Dólares estadounidenses ($)</option>
                            </select>
                            @error('currency')
                                <span class="invalid-feedback d-block" role="alert">
                                    <strong>{{ $message }}</strong>
