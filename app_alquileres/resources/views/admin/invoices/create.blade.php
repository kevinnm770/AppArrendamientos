    ...
    <section class="section">
        ...
        <div id="payment-receipt-modal-backdrop" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,.5); z-index:1055; align-items:center; justify-content:center;">
            <div class="card" style="width: 100%; max-width: 860px; max-height: 90vh; overflow-y: auto;">
                <div class="card-body">
                    ...
                    <div class="alert alert-info d-none mt-2 py-2 px-3 mb-0" id="pr-excess-banner">
                        <span id="pr-excess-text"></span>
                        <button type="button" class="btn btn-sm btn-outline-info ms-2" id="pr-excess-grant-btn">Otorgar saldo a favor</button>
                    </div>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                // Última respuesta de admin.agreements.tenant-balance, usada tanto para
                // sugerir "Otorgar saldo a favor" en el panel (saldos negativos por concepto)
                // como para calcular si el comprobante de pago embebido excede lo pendiente.
                ...
                // --- Modal "Otorgar saldo a favor", compartido por el botón del panel de
                // Saldos y el aviso de excedente dentro del comprobante de pago embebido. Se
                // envía por fetch (no como <form> normal) para no perder ni la factura ni el
                // comprobante que el usuario ya esté llenando.
                ...
                document.getElementById('pr-excess-grant-btn').addEventListener('click', function() {
                    openGrantModal(parseFloat(prExcessBanner.dataset.excessAmount) || 0, 'Excedente registrado en un comprobante de pago');
                });
                ...
                const prExcessBanner = document.getElementById('pr-excess-banner');
                const prExcessText = document.getElementById('pr-excess-text');

                // Mismo cálculo que updateExcessBanner() en payment-receipts/create.blade.php:
                // por concepto (sin líneas "Aplicar saldo a favor", que no son dinero nuevo)
                // contra el saldo pendiente de cada concepto antes de este comprobante.
                function prUpdateExcessBanner() {
                    if (!lastTenantBalanceData) {
                        prExcessBanner.classList.add('d-none');
                        return;
                    }

                    const paidByConcept = {};
                    prItems.forEach(function(item) {
                        if (item.is_credit_application) return;
                        paidByConcept[item.concept] = (paidByConcept[item.concept] || 0) + prComputeLineTotal(item);
                    });
                    ...
                    if (excess > 0) {
                        prExcessText.textContent = `Este comprobante registra ${lastTenantBalanceData.currency} ${excess.toFixed(2)} de más sobre lo pendiente.`;
                        prExcessBanner.dataset.excessAmount = excess;
                        prExcessBanner.classList.remove('d-none');
                    } else {
                        prExcessBanner.classList.add('d-none');
                    }
                }
                ...
                function prRenderItems() {
                    ...
                    prItems.forEach(function(item, idx) {
                        ...
                        if (isCreditApplication) {
                            row.innerHTML = `
                                <td>
                                    <input type="hidden" class="pr-item-description" value="${item.description || 'Aplicación de saldo a favor'}">
                    ...
                    paymentReceiptDraft = {
                        ...
                        items: (oldPaymentReceipt.items || []).map(function(item) {
                            return {
                                ...
                                // old('payment_receipt') trae el valor del POST tal cual (string
                                // "0"/"1"): "0" es truthy en JS, así que hay que comparar contra
                                // el string en vez de castear con !! directamente.
                                is_credit_application: item.is_credit_application === '1' || item.is_credit_application === true,
